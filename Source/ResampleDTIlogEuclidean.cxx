...
#include "ResampleDTIlogEuclideanCLP.h"
...
int Do( parameters list )
{
  ...
  if( ( list.transformationFile.compare( "" )
    ...
    // Save HField
    if( list.outputDisfield.compare( "" ) )
      {
      typedef itk::ImageFileWriter<DeformationImageType> FieldWriterType;
      typedef typename FieldWriterType::Pointer          FieldWriterTypePointer;
      FieldWriterTypePointer fieldwriter = FieldWriterType::New();
      fieldwriter->SetInput( field );
      fieldwriter->UseCompressionOn();
      fieldwriter->SetFileName( list.outputDisfield );
      try
        {
        fieldwriter->Update();
        }
        ...
        {
        std::cerr << "Writing displacement field: Exception caught!"
                  << std::endl;
        std::cerr << Except << std::endl;
        return EXIT_FAILURE;
        }
  ...
  // If log-Euclidean framework(default): compute log of the tensors
  double defaultPixelValue = list.defaultPixelValue;
  if( !list.nolog )
    {
    typedef itk::DiffusionTensor3DLogImageFilter<InputImageType, InputImageType>
      LogFilterType;
    typename LogFilterType::Pointer logFilter;
    logFilter = LogFilterType::New();
    logFilter->SetInput( image );
    if( list.numberOfThread )
      {
      logFilter->SetNumberOfThreads( list.numberOfThread );
      }
    logFilter->Update();
  ...
  // If log-Euclidean Framework: we go back in the original space by taking the exponential of the tensors
  if( !list.nolog )
    {
    typedef itk::DiffusionTensor3DExpImageFilter<InputImageType, InputImageType>
      ExpFilterType;
    typename ExpFilterType::Pointer expFilter;
    expFilter = ExpFilterType::New();
    expFilter->SetInput( image );
    if( list.numberOfThread )
      {
      expFilter->SetNumberOfThreads( list.numberOfThread );
      }
    expFilter->Update();
